...
port = int(input('port:'))
server_socket.bind(('192.168.35.182', port))
...
# sennder = threading(target=send_sock, args=(client_socket))
# receiever =threading.Thread(target=recv_sock, args=(client_socket))
